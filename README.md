...
- ただし DevTools ページの起動時だけは、設定を読み終える前から購読を張る。設定の読み込みは非同期であり、それを待ってから購読すると、ページの読み込み中に DevTools を開いた場合に最初の数件を取りこぼす。この間に拾った分は、保存の直前で初回読み込みの完了を待ってから可否を判断する（記録が OFF なら捨てる）。
- 設定を「読み込んだ値」と「変更通知」の両方から受け取る箇所（DevTools ページ・popup）では、通知が先に届いた場合に後から解決した読み込み結果で巻き戻さない。読み込みの完了は通知より遅れうるため。
...
- 自動更新には歯止めを 3 つ置く。URL 部分一致のようにインデックスで表現できない条件では 1 回の取得がストアの全走査になりうるため、**前回の取得が終わるまで次を始めない**（間引く対象は自動更新だけで、条件変更と手動更新は間引かない）。**パネルが見えていない間は取りに行かない**（`document.hidden`。再表示時は次の間隔を待たずに追いつく）。**結果の並びが前回と同じなら state を更新しない**（`hasSameLogs()`。`logs` は追記専用でメタデータが後から変わらないため ID の並びで判定できる。無駄な再描画で開いている詳細のボディ整形まで再実行されるのを防ぐ）。
- フィルタは入力のたびではなく「適用」または Enter で確定する。数字でないステータスや解釈できない日時は、条件そのものを積まない（誤入力で 0 件になるより、絞り込まずに見せるほうが調査中は穏当なため）。
- 期間の上限は入力の粒度いっぱいまで含める。`<input type="datetime-local">` は既定で分単位のため、素直に解釈すると終端が `:00.000` になり、`12:31` を指定したとき 12:31:20 の記録が黙って外れる（`parseRangeEnd()`）。
- ボディは行を選択したときに初めて `getBody()` で取得する。一覧では読まない（8.3）。取得結果は「どのログのものか」を一緒に持ち、描画時に選択と突き合わせて捨てる。effect で state をクリアする作りだと、行を切り替えた直後の 1 フレームに「新しいエントリのヘッダー＋前のエントリのボディ」が出てしまうため。
- ボディの JSON 整形は最大 1MB の `JSON.parse` + `JSON.stringify` になるため、ボディが変わったときだけ計算する（自動更新のたびにやり直さない）。
- 破棄したヘッダー名（`droppedRequestHeaders` / `droppedResponseHeaders`）も詳細に表示する。値は保存していないため名前だけを出し、リクエストとレスポンスは行を分ける（同名でも意味が違うため）。
- 詳細ペインは横並び（広いとき）では固定幅、縦積み（DevTools を右にドックしたときなど狭いとき）では一覧と高さを分け合って自前でスクロールする。高さを詰めないとページ全体の `overflow: hidden` に切られ、ボディまで辿り着けなくなる。
