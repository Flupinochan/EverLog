/**
 * Network タブの中身。フック（データ取得）と表示コンポーネントを繋ぐ層。
 *
...
interface Props {
  ...
  /** 拡張機能のバージョン。HAR の `creator.version` に載せる */
  version: string;
  /**
   * このタブが表示されているか。
   *
   * 隠れている間は自動更新を止める。タブを切り替えただけで見ていない一覧の
   * 全走査が回り続けるのを避ける。入力内容を残すため、マウント自体は続ける。
   */
  active: boolean;
  /** 自動更新の間隔（ミリ秒） */
...
export function NetworkView({ source, tabId, version, active, refreshIntervalMs }: Props) {
  ...
  const filter = useMemo(() => buildFilter(applied, tabId), [applied, tabId]);
  ...
  const exportHar = useCallback(() => {
    apply();
    startExport(buildFilter(form, tabId));
