/**
 * コンソール側のフィルタ欄。props で受けた値を描くだけの表示コンポーネント。
 * 条件の解釈は `@/lib/panel-view` の `buildConsoleFilter()` が担う。
 */

import {
  CONSOLE_LEVEL_OPTIONS,
  describeConsoleLevel,
  toggleLevel,
  type ConsoleFilterForm,
} from '@/lib/panel-view';

interface Props {
  form: ConsoleFilterForm;
  onChange: (patch: Partial<ConsoleFilterForm>) => void;
  /** 入力を確定して問い合わせる */
  onApply: () => void;
  onReset: () => void;
  /** 検査中のタブが分からない場合は「このタブのみ」を選べない */
  tabFilterAvailable: boolean;
  autoRefresh: boolean;
  onAutoRefreshChange: (enabled: boolean) => void;
  onReload: () => void;
  /** 現在のフィルタに一致する全件を JSON で出力する */
  onExport: () => void;
  exporting: boolean;
}

const CONTROL =
  'h-6 rounded border border-zinc-300 bg-white px-1.5 text-xs text-zinc-900 ' +
  'focus:border-sky-500 focus:outline-none dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-100';

const BUTTON =
  'h-6 rounded border border-zinc-300 bg-zinc-100 px-2 text-xs text-zinc-700 ' +
  'hover:bg-zinc-200 dark:border-zinc-600 dark:bg-zinc-700 dark:text-zinc-100 dark:hover:bg-zinc-600';

export function ConsoleFilterBar({
  form,
  onChange,
  onApply,
  onReset,
  tabFilterAvailable,
  autoRefresh,
  onAutoRefreshChange,
  onReload,
  onExport,
  exporting,
}: Props) {
  return (
    <form
      className="flex flex-wrap items-center gap-1.5 border-b border-zinc-200 bg-zinc-50 px-2 py-1.5 dark:border-zinc-700 dark:bg-zinc-800"
      onSubmit={(event) => {
        event.preventDefault();
        onApply();
      }}
    >
      <input
        type="search"
        className={`${CONTROL} w-56`}
        placeholder="本文に含まれる文字列"
        value={form.textIncludes}
        onChange={(event) => onChange({ textIncludes: event.target.value })}
      />
      <input
        type="search"
        className={`${CONTROL} w-44`}
        placeholder="ページ URL"
        value={form.pageUrlIncludes}
        onChange={(event) => onChange({ pageUrlIncludes: event.target.value })}
      />
      <input
        type="datetime-local"
        className={CONTROL}
        value={form.from}
        onChange={(event) => onChange({ from: event.target.value })}
      />
      <span className="text-xs text-zinc-500">〜</span>
      <input
        type="datetime-local"
        className={CONTROL}
        value={form.to}
        onChange={(event) => onChange({ to: event.target.value })}
      />
      <label
        className="flex items-center gap-1 text-xs text-zinc-700 dark:text-zinc-300"
        title={tabFilterAvailable ? undefined : '検査中のタブを特定できません'}
      >
        <input
          type="checkbox"
          checked={form.onlyCurrentTab}
          disabled={!tabFilterAvailable}
          onChange={(event) => onChange({ onlyCurrentTab: event.target.checked })}
        />
        このタブのみ
      </label>

      <button type="submit" className={BUTTON}>
        適用
      </button>
      <button type="button" className={BUTTON} onClick={onReset}>
        クリア
      </button>
      <button
        type="button"
        className={`${BUTTON} disabled:opacity-50`}
        onClick={onExport}
        disabled={exporting}
        // 表示中の件数ではなく条件に一致する全件が出るため、ここで明示しておく
        title="現在のフィルタに一致する全件を JSON ファイルとして出力します"
      >
        {exporting ? '出力中…' : 'Console 出力'}
      </button>

      <span className="ml-auto flex items-center gap-1.5">
        <label className="flex items-center gap-1 text-xs text-zinc-700 dark:text-zinc-300">
          <input
            type="checkbox"
            checked={autoRefresh}
            onChange={(event) => onAutoRefreshChange(event.target.checked)}
          />
          自動更新
        </label>
        <button type="button" className={BUTTON} onClick={onReload}>
          更新
        </button>
      </span>

      {/* レベルは件数が多いので行を分ける。1 つも選ばない状態を「絞らない」とする */}
      <div className="flex w-full flex-wrap items-center gap-x-2.5 gap-y-1 pt-0.5">
        <span className="text-xs text-zinc-500 dark:text-zinc-400">レベル</span>
        {CONSOLE_LEVEL_OPTIONS.map((level) => (
          <label
            key={level}
            className="flex items-center gap-1 font-mono text-xs text-zinc-700 dark:text-zinc-300"
          >
            <input
              type="checkbox"
              checked={form.levels.includes(level)}
              onChange={(event) =>
                onChange({ levels: toggleLevel(form.levels, level, event.target.checked) })
              }
            />
            {describeConsoleLevel(level)}
          </label>
        ))}
        {form.levels.length === 0 && (
          <span className="text-xs text-zinc-400 dark:text-zinc-500">（未選択はすべて表示）</span>
        )}
      </div>
    </form>
  );
}
