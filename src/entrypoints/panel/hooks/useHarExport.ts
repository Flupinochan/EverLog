...
import { HAR_MIME_TYPE, buildHar, estimateExportBytes, harFileName } from '@/lib/har';
...
/**
 * @param source データ取得先
 * @param filter 出力対象の条件。一覧と同じものを渡す（呼び出し側で memo 化しておく）
...
export function useHarExport(
  ...
  const write = useCallback(
    async (logs: StoredLog[]) => {
      ...
      downloadText(JSON.stringify(har), harFileName(Date.now()), HAR_MIME_TYPE);
    },
